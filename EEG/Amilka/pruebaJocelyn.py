...
import householder as householder
import time as time 
import os
...
def getNextSquareRoot(P,Q,F, typeM):
    ...
    if (typeM == 'initial'):
        ...
        try:
            ...
            SnewT = householder.householder_filter(F, Q, L)
        ...
        try:
            SnewT = householder.householder_filter(F, Q, P)
        except np.linalg.LinAlgError:
            SnewT = 0
            return SnewT
...
########################## CARLSON ##########################
def carlsonFilter(prevS, H, R, x, y):
    n = len(x)
    m = len(y)
    x_Prev = x
    
    if (np.allclose(prevS, np.tril(prevS))): #if is low triangle, do not change it
        S_prev = prevS
    else:
        S_prev = prevS.T
    ...
    for j in range(0, m):
        S = np.zeros([n,n])
        H_j = H if H.shape == (1,len(H)) else np.reshape(H[j], (1,n))
        phi = np.reshape(np.dot(S_prev, H_j.T), (n,1))
        d_Prev = np.var(R[j])
        e_Prev = np.zeros([n,1])
        
        for i in range(0,n):
            d_Next = d_Prev + math.pow(phi[i],2)
            b = math.sqrt(d_Prev / d_Next)
            c = float(phi[i] /math.sqrt(d_Prev * d_Next))
            e_Next = e_Prev + (np.reshape(S_prev.T[:,i] * phi[i], (n,1)))
            tempColumn = np.reshape(S_prev.T[:,i] * b, (n,1)) - (e_Prev * c)
            S[:,i] = np.reshape(tempColumn, (n,))
            d_Prev = d_Next
            e_Prev = e_Next
        
        temp = (y[j] - (H_j.dot(x_Prev))) / d_Prev 
        x_P = x_Prev + (e_Prev.dot(temp))
...
def EnKF(name_Signal, samplingRate, wC):
    ...
    for i in range(len(signal)):
        ...
        for j in range(samplingRate):
           ...
           pk, x_preAll = carlsonFilter(S_t,H,R,xpt,y)
           pk_WC, x_preWC = carlsonFilter(S_tWC,H_WC,R_WC,xpt_WC,yWC)
           pk_NWC, x_preNWC = carlsonFilter(S_tNWC,H_NWC,R_NWC,xpt_NWC,yNWC)
           ty = 'other'        
...
# Define path and check if header exists
time_log_path = os.path.join(script_dir, f"time_per_session_{name}.txt")
write_header = not os.path.exists(time_log_path)
with open(time_log_path, "a") as log_file:
    if write_header:
        log_file.write("Session\tMethod\tTimeTaken(s)\n")  # add header only once
...
for m in range(2,21): #SESSIONS TO ANALYZE
    ...
    with open(time_log_path, "a") as log_file:
        log_file.write(f"{m}\tCarlsonxHouseholder\t{session_duration:.2f}\n")
